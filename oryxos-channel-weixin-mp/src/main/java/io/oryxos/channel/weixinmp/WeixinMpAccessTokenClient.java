...
final class WeixinMpAccessTokenClient {
  ...
  WeixinMpAccessTokenClient(OutboundGuard guard, String appId, String secret) {
    this(HttpClient.newBuilder().connectTimeout(TIMEOUT).build(), guard, appId, secret);
  }

  WeixinMpAccessTokenClient(HttpClient http, OutboundGuard guard, String appId, String secret) {
    this.http = http;
    this.guard = guard;
    this.appId = appId;
    this.secret = secret;
    ...
    try {
      ...
      if (response.statusCode() < HTTP_OK_MIN || response.statusCode() >= HTTP_OK_MAX_EXCLUSIVE) {
        throw new IllegalStateException(
            "服务号 token HTTP " + response.statusCode() + ": " + sanitize(response.body()));
      }
      JsonNode root = MAPPER.readTree(response.body() == null ? "{}" : response.body());
      int errcode = root.path("errcode").asInt(0);
      if (errcode != 0) {
        throw new IllegalStateException(
            "服务号 token errcode=" + errcode + " errmsg=" + root.path("errmsg").asText());
      }
      String token = root.path("access_token").asText(null);
      if (token == null || token.isBlank()) {
        throw new IllegalStateException("服务号 token 未返回 access_token");
      }
      ...
      throw new IllegalStateException("服务号 token 失败: " + e.getMessage(), e);
    }
  }
