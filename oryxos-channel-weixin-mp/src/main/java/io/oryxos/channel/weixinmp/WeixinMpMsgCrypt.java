...
/**
 * 公众平台回调加解密（WXBizMsgCrypt）：Token + EncodingAESKey + receiveId(AppId)。
 *
...
@edu.umd.cs.findbugs.annotations.SuppressFBWarnings(
    ...
    justification =
        "公众平台回调协议固定 AES-256-CBC + msg_signature=SHA1（官方 WXBizMsgCrypt），"
            + "无法改用 AEAD/SHA-256；完整性依赖平台侧签名字段。")
final class WeixinMpMsgCrypt {
  ...
  WeixinMpMsgCrypt(String token, String encodingAesKey, String receiveId) {
