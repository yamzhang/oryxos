...
/** 服务号回复窗：用户主动发信后 48h / 每回合最多 5 条下行。 */
final class WeixinMpReplySessionStore {
  ...
  Session requireForReply(String chatId, long nowMs) {
    ...
    if (session == null) {
      throw new IllegalStateException("服务号会话上下文缺失，需用户先发消息（chat=" + chatId + "）");
    }
    if (nowMs > session.expiresAtMs()) {
      byChatId.remove(chatId, session);
      throw new IllegalStateException("服务号 48 小时回复窗已关闭，需用户再发一条消息（chat=" + chatId + "）");
    }
    if (session.replyCount() >= MAX_REPLIES_PER_WINDOW) {
      throw new IllegalStateException(
          "服务号回复条数已达上限（" + MAX_REPLIES_PER_WINDOW + "），需用户再发一条消息（chat=" + chatId + "）");
    }
    return session;
  }
